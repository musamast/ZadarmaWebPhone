...
class API(object):
    ...
    def call(self, method, params={}, request_type='GET', format='json', is_auth=True):
        ...
        for k in params.values():
            if not isinstance(k, str):
                ...
                break
        if is_nested_data:
            params_string = self.__http_build_query(
                OrderedDict(sorted(params.items())))
        ...
        if request_type == 'GET':
            result = requests.get(self.__url_api + method + '?' +
                                  params_string, headers={'Authorization': auth_str})
        elif request_type == 'POST':
            result = requests.post(
                self.__url_api + method, headers={'Authorization': auth_str}, data=params)
    ...
    def __get_auth_string_for_header(self, method, params_string):
        ...
        :return: auth header
        """
        data = method + params_string + \
            md5(params_string.encode('utf8')).hexdigest()
        hmac_h = hmac.new(self.secret.encode('utf8'),
                          data.encode('utf8'), sha1)
        if sys.version_info.major > 2:
